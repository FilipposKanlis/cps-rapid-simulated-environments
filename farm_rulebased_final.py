...
print("="*60)
print(f"SMART FARM PATTERN RECOGNITION SYSTEM")
...
CONSTRAINTS = {
    "forbidden": [
        ["AA:lake_alarm:activated", "requires_before", "LA:smartcows_alarm:activated"],
        ["LA:smartgarden_alarm:activated", "requires_one_of_before", [
            "LA:_fieldwithtractor_alarm:activated",
            "LA:appletrees_alarm:activated"
        ]]
    ],
    "mandatory": [
        ["LA:entrance_alarm:activated"],
        ["LA:smartgarden_alarm:activated", "one_of_before", [
            "LA:_fieldwithtractor_alarm:activated",
            "LA:appletrees_alarm:activated"
        ]],
        ["AA:lake_alarm:activated", "AA:lake_alarm:deactivated"]
    ],
    "temporal": [
        {"pair": ["LA:_fieldwithtractor_alarm:activated", "LA:smartgarden_alarm:activated"], "max_seconds": 900},
        {"pair": ["LA:appletrees_alarm:activated",       "LA:smartgarden_alarm:activated"], "max_seconds": 900},
        {"pair": ["LA:smartcows_alarm:activated",        "AA:lake_alarm:activated"],        "max_seconds": 600},
        {"pair": ["LA:entrance_alarm:activated",         "*:*:activated"],                  "max_seconds": 180}
    ],
    "hard_negatives": []
}
...
FILIPPOS_PATTERNS = {
    "pattern1": [
        "LA:entrance_alarm:activated",
        "LA:_fieldwithtractor_alarm:activated",
        "LA:smartgarden_alarm:activated",
    ],
    "pattern2": [
        "LA:entrance_alarm:activated",
        "LA:smartcows_alarm:activated",
        "AA:lake_alarm:activated",
        "AA:lake_alarm:deactivated",
    ],
    "pattern3": [
        "LA:entrance_alarm:activated",
        "LA:appletrees_alarm:activated",
...
class PatternMatcher:
    ...
    def _calculate_window_size(self) -> int:
        ...
        return min(max(WINDOW_SIZE, int(max_len * 2.0)), 200)
    ...
    def add_event(self, symbol: str, timestamp: float) -> Dict[str, Any]:
            ...
            # Debounce: skip consecutive identical events
    
            if self.event_buffer and self.event_buffer[-1] == symbol:
            ...
            # 2) Rolling F1 comparison across patterns
            ...
            comparison_result = {
                ...
                "detection": "Other/Unknown",
    
            }
    
            obs_tokens = buffer_list
            ...
            for pattern_name, pattern_steps in self.patterns.items():
                ...
                metrics = compute_sequence_metrics(pattern_steps, obs_tokens)
    
                score = metrics.get("f1_overall", 0.0)
            ...
            for pname, m in comparison_result["matches"].items():
    
                detection_stats["pattern_scores"][pname].append(m["score"])
...
def create_compact_detection_report():
    ...
    header_text = fig.text(0.5, 0.96, f'SMART {ENVIRONMENT.upper()} PATTERN RECOGNITION', 
    ...
    fig.suptitle(f'SMART {ENVIRONMENT.upper()} PATTERN RECOGNITION - {SESSION_ID}', fontsize=14, fontweight='bold', y=0.98)
...
TOPICS = [
    # Linear alarms
    f"streamsim.{UID}.world.world.sensor.alarm.linear_alarm.entrance_alarm.data",
    f"streamsim.{UID}.world.world.sensor.alarm.linear_alarm.smartgarden_alarm.data",
    f"streamsim.{UID}.world.world.sensor.alarm.linear_alarm.appletrees_alarm.data",
    f"streamsim.{UID}.world.world.sensor.alarm.linear_alarm._fieldwithtractor_alarm.data",
    f"streamsim.{UID}.world.world.sensor.alarm.linear_alarm.smartcows_alarm.data",

    # Area alarms (περιοχές/ζώνες — περιλαμβάνεται και η λίμνη)
    ...
    f"streamsim.{UID}.world.world.sensor.alarm.area_alarm.lake_alarm.data",
